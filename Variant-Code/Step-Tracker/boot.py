...
This variant includes an option to track the steps the micrometer is on and update the position in the tracking textfile.
Recommended so that you do not overextend the micrometer and therefore the code does not run if the step count will go over (or under).
...
def f_step(n,dmin=0.002,dstart=0.003,gradient=0.0001):
    ...
    f.close()
    new_pos = pos-n
    #make sure steps don't go under 0
    if new_pos >=0:    
        for i in range(n):
        ...
        p4.off()
        p5.off()
        #overwrite tracking textfile with new position
        f = open("track.txt", "w")
        f.write(str(new_pos))
        f.close()
    else:
        print('Error: micrometer out of bounds')
...
def b_step(n,dmin=0.002,dstart=0.003,gradient=0.0001):
    ...
    f.close()
    new_pos = pos+n
    #make sure steps won't go over 24576
    if new_pos<=24576:
        for i in range(n):
            # Check if ramp down required to reach starting speed with number of steps left.
            if (n-i)*gradient + dmin <= dstart:
